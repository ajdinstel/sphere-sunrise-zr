...
public class ProductDetailPageContentTest {
    ...
    @Test
    public void shippingRateJson() throws IOException {
    ...
    @Test
    public void pdpJson() throws IOException {
        final CmsPage cms = (messageKey, args) -> Optional.of(messageKey);
        final PdpStaticData pdpStaticData = new PdpStaticData(cms, BagItemDataFactory.of().create(100), RatingDataFactory.of(cms).create());
        ...
        final ShopShippingRate dhlFreeAbove = new ShopShippingRate("DHL", ShippingRate.of(Money.of(10, "EUR"), Money.of(50, "EUR")));
        final ShippingRateDataFactory shippingRateDataFactory = ShippingRateDataFactory.of(priceFormatter);
        final List<ShippingRateData> shippingRateData = asList(dhl, dhlFreeAbove).stream().map(shippingRateDataFactory::create).collect(toList());
        ...
        final ProductDataFactory productDataFactory = ProductDataFactory.of(userContext);
        final List<ProductData> suggestionData = asList(selma, dkny, miabag, altea).stream()
                .map(p -> productDataFactory.create(p, p.getMasterVariant())).collect(toList());

        final JsonNode expected = readJsonNodeFromResource("pdpContent.json").get("content");
        final JsonNode result = toJsonNode(new ProductDetailPageContent("", pdpStaticData, breadcrumbData, productData, shippingRateData, suggestionData));
